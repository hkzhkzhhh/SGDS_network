...
class RoadDataset(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in batch_indices:
            ...
            label = cv2.imread(os.path.join(self.folder, "label", self.label_paths[i]), cv2.IMREAD_GRAYSCALE) / 255.0
...
# 定义ResUNet模型
def resunet_model(input_shape=(256, 256, 3)):
    inputs = layers.Input(shape=input_shape)

    def residual_block(x, filters):
        shortcut = layers.Conv2D(filters, (1, 1), padding="same")(x)  # 调整维度以匹配
