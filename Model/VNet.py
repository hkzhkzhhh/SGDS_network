...
def conv_block(x, filters, kernel_size=(3, 3), padding="same", strides=1):
    x = layers.Conv2D(filters, kernel_size=kernel_size, padding=padding, strides=strides)(x)
    x = layers.BatchNormalization()(x)
    x = layers.ReLU()(x)
    return x

# 定义 V-Net 模型
def vnet(input_shape=(256, 256, 3)):
    ...
    # Decoder
    u4 = layers.Conv2DTranspose(128, (2, 2), strides=(2, 2), padding="same")(c5)
    u4 = layers.concatenate([u4, c4])
    u4 = conv_block(u4, 128)
    u4 = conv_block(u4, 128)

    u3 = layers.Conv2DTranspose(64, (2, 2), strides=(2, 2), padding="same")(u4)
    u3 = layers.concatenate([u3, c3])
    u3 = conv_block(u3, 64)
    u3 = conv_block(u3, 64)

    u2 = layers.Conv2DTranspose(32, (2, 2), strides=(2, 2), padding="same")(u3)
    u2 = layers.concatenate([u2, c2])
    u2 = conv_block(u2, 32)
    u2 = conv_block(u2, 32)

    u1 = layers.Conv2DTranspose(16, (2, 2), strides=(2, 2), padding="same")(u2)
